...
public class HCwithPerp3 {
    ...
    public HCwithPerp3(TopicDistributions Phi,  double AlphaPrecision , Perplexity perplexity , String root, int Step, int GroupMaxMinSize) {
        this.Phi = Phi;
    ...
    public void evaluate(int s) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        for(int size=1; size<= maxSize; size++) {
            ...
            if (keys.size()>0) {
                ...
                Prior newAlphaPrior = new Prior(AlphaPrecision,keys.size());
                double perp = perplexity.LeftToRight(newAlphaPrior, newPhi);
                System.out.println("minimum cluster size "+size+"("+ keys.size()+"topics:"+perp+" alpha_k:"+newAlphaPrior.getParam(0));
